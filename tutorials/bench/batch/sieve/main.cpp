/*
This program finds all the prime numbers less than a certain number
using the sieve of Eratosthenes algorithm. It takes a number as input 
and returns a vector of all the prime numbers less than this number.
*/
...
void printVector(const vector<T> & v)
{
    ...
    for (size_t p = 2; p < v.size(); p++)
        //Print the first 10 and last 10 elements
        if ( p < 10 || p + 10 > v.size() )
            cout << ' ' << de(v[p]);
        else
        { 

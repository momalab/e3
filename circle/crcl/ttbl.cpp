...
void tt2Circle(std::istream & in, string file)
{
...
bool checkFmt(string file, std::istream & in)
{
    bool ari = false;
    char c;
...
void writeTT(string ofile, const Module & prog, bool arith)
{
    int sz = prog.input_size();
    if (sz < 1) throw "Empty circle";
...
void verify(string fil1, string fil2, bool brnd, string extra, bool nonly)
{
    ...
    if (brnd) return verify(prog1, prog2, sz, szo, extra, fil1, fil2, nonly);

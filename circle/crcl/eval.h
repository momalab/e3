...
vector<Vx> Module::eval(Visitor & vis, const vector<Vx> & inps, bool threaded) const
{
    ...
    if ( !threaded )
    {
        ...
        std::map<string, Vx> mlocal;
        if ( !vis.m ) vis.m = &mlocal; // let Visitor collect info
        std::map<string, Vx> & m = *vis.m;
        ...
        vector<Vx> r(outputs.size(), vis.zero());
        i = 0;

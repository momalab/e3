    ...
    : filename(file), flat(!noflatten), reuse(false)
{
    ...
    if (++depth > MAXDEPTH)
        ...
        "max depth (" + ol::tos(MAXDEPTH) + ") is reached";
    {
...
vector<Statement *> Statement::load(string line)
{
    ...
    return bind_ios(mod.inputs, st.iexpr.args.names, true)
           + mod.statements
           + bind_ios(st.oexpr.args.names, mod.outputs, false);
}

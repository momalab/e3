...
bool e3util::isHex(const std::string & hex)
{
    for (auto c : hex)
    {
        if ( !( (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') ) )
...
string e3util::bin2hex(const std::vector<bool> & bin)
{
...
std::vector<bool> e3util::hex2bin(const string & hex, size_t bitsize)
{
    std::vector<bool> bin;
    if (isHex(hex))
    {
...
unsigned e3util::hex2dec(char hex)
{
    if ( hex <= '9' ) return hex - '0';
    return hex - 'a' + 10;
}

char e3util::hex2dec(char hex, bool * carry)
{
    bool c = false;
    char dec;
    if ( hex >= 'a' )
...
string e3util::hex2dec(const string & hexStr, size_t bitsize, bool sign)
{
    string hex (hexStr);
...
char e3util::dec2hex(unsigned dec)
{
    if ( dec <= 9 ) return (char)(dec + '0');
    return (char)(dec - 10 + 'a');
}

string e3util::dec2hex(const string & decStr, size_t bitsize)
{
...
string e3util::twosComplement(const string & hex, size_t bitsize)
{
    return increment(negation(hex, bitsize), bitsize);
}

string e3util::increment(const string & hex, size_t bitsize)
{
    string sum;
    bool c = false;
...
string e3util::negation(const string & hexStr, size_t bitsize)
{
    string hex (hexStr);
...
string e3util::add(const string & dec, unsigned value)
{
    string r;
...
string e3util::multiply(const string & dec, unsigned value)
{
    string r;
    unsigned carry = 0;
    for ( int i = (int)dec.size() - 1; i >= 0; i-- )
...
string e3util::removeLeadingZeros(const string & s)
{
    string r;
    bool start = false;
    for ( auto c : s )
...
char e3util::addHex(char hex1, char hex2, bool * carry)
{
    bool c = false;
...
string e3util::divide(const string & dec, unsigned div)
{
    string r;
    unsigned i = 0;
    unsigned t = dec[i] - '0';
...
unsigned e3util::mod(const string & dec, unsigned div)
{
    unsigned r = 0;
...
char e3util::maskHex(char c, size_t bitsize)
{
    auto uppersize = bitsize % 4;
    uppersize = uppersize ? uppersize : 4;
...
string e3util::zeroExtension(const string & hex, size_t bitsize)
{
...
string e3util::bin2sbn(const std::vector<bool> & bin)
{
...
std::vector<bool> e3util::sbn2bin(const string & sbin, size_t bitsize)
{
